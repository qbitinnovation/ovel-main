...
export default function WeekendPricingPage() {
  ...
  const mergeRules = (
    ...
    if (slotsMap.size === 0) {
      return DEFAULT_WEEKEND_SLOTS.map((d, idx) => ({
        id: `default-we-${idx}`,
        name: d.name,
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        const daysSetting = settings.find((s) => s.key === 'turf_weekend_days');
        if (daysSetting && Array.isArray(daysSetting.value)) {
          setWeekendDays(daysSetting.value as number[]);
        }

        const turfRules = settings.find((s) => s.key === 'turf_weekend_rules')?.value as TurfSlotPriceRule[] || [];
        const machineRules = settings.find((s) => s.key === 'nets_machine_weekend_rules')?.value as TurfSlotPriceRule[] || [];
        const nomachineRules = settings.find((s) => s.key === 'nets_nomachine_weekend_rules')?.value as TurfSlotPriceRule[] || [];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      unifiedRules.forEach((ur, idx) => {
        const baseId = ur.id.includes('unified-slot') || ur.id.includes('default-we')
          ? `rule-we-${idx}-${Date.now()}`
          : ur.id;
      ...
      const res = await fetch('/api/settings', {
        ...
        body: JSON.stringify({
          settings: [
            ...
            { key: 'turf_weekend_rules', value: turfRules },
            { key: 'nets_machine_weekend_rules', value: netsMachineRules },
            { key: 'nets_nomachine_weekend_rules', value: netsNoMachineRules },
          ]
        }),
      });
    ...
    <div className="page-container">
      ...
      <div className="page-header" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
        ...
        <div style={{ marginTop: 'var(--space-2)' }}>
          ...
          <p className="page-subtitle">Select weekend days and configure hourly pricing rates side-by-side</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', marginTop: 'var(--space-4)' }}>
          {/* Days selector */}
          <div className="card" style={{ border: '1px solid var(--surface-glass-border)', boxShadow: 'var(--shadow-sm)' }}>
          ...
          {/* Pricing matrix slots */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 'var(--space-4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
              <HelpCircle size={14} />
              <span>Configure weekend hours and rates across different facility types side-by-side. 12-hour AM/PM selection is supported.</span>
            </div>
            <button
              type="button"
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-3)', alignItems: 'center', borderBottom: '1px solid var(--surface-glass-border)', paddingBottom: 'var(--space-3)' }}>
                        <div style={{ flex: '1 1 200px' }}>
                          ...
                          <input
                            ...
                            placeholder="e.g. Peak Weekend Slot"
