...
export default function WeekdayPricingPage() {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      if (data.success) {
        const rulesSetting = (data.data as Setting[]).find((s) => s.key === 'turf_weekday_rules');
        if (rulesSetting && Array.isArray(rulesSetting.value)) {
          if (rulesSetting.value.length === 0) {
            const defaultRules = [
              { id: 'rule-wd-1', name: '6AM-10AM', startTime: '06:00', endTime: '10:00', normalPricePerHour: 2420, regularPricePerHour: 2200, dayType: 'weekdays' as const, isActive: true },
              { id: 'rule-wd-2', name: '10AM-4PM', startTime: '10:00', endTime: '16:00', normalPricePerHour: 1870, regularPricePerHour: 1650, dayType: 'weekdays' as const, isActive: true },
              { id: 'rule-wd-3', name: '4PM-6PM', startTime: '16:00', endTime: '18:00', normalPricePerHour: 2420, regularPricePerHour: 2200, dayType: 'weekdays' as const, isActive: true },
              { id: 'rule-wd-4', name: '6PM-12AM', startTime: '18:00', endTime: '00:00', normalPricePerHour: 2750, regularPricePerHour: 2530, dayType: 'weekdays' as const, isActive: true }
            ];
            setWeekdayRules(defaultRules);
            setHasChanges(true);
          } else {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/settings', {
        ...
        body: JSON.stringify({
          settings: [
            { key: 'turf_weekday_rules', value: weekdayRules }
          ]
        }),
      });
      const data = await res.json();
  ...
  const updateRule = (index: number, patch: Partial<TurfSlotPriceRule>) => {
    setWeekdayRules((prev) => prev.map((rule, idx) => idx === index ? { ...rule, ...patch } : rule));
    setHasChanges(true);
  };
  ...
  const removeRule = (index: number) => {
    setWeekdayRules((prev) => prev.filter((_, idx) => idx !== index));
    setHasChanges(true);
  };
          ...
          <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 'var(--space-4)' }}>
            <div>
              <h3 style={{ fontSize: 'var(--text-sm)', display: 'flex', alignItems: 'center', gap: 'var(--space-2)', margin: 0 }}>
                <Calendar size={16} /> <span>Weekday Price Customization Slots</span>
              </h3>
            </div>
            <button
              type="button"
